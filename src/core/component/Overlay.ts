...
export type OverlayFigureIgnoreEventType =
  | 'mouseClickEvent'
...
export interface Overlay {
   ...
   */
  performEventPressedMove: Nullable<
    (params: OverlayPerformEventParams) => void
  >;

  /**
   * In drawing, special handling callback when moving events
   */
  performEventMoveForDrawing: Nullable<
    (params: OverlayPerformEventParams) => void
  >;
...
export type OverlayTemplate = ExcludePickPartial<
  Omit<Overlay, 'id' | 'groupId' | 'paneId' | 'points' | 'currentStep'>,
  'name'
>;

export type OverlayCreate = ExcludePickPartial<
  Omit<
    Overlay,
    | 'currentStep'
    | 'totalStep'
...
export default class OverlayImp {
  ...
  override(overlay: Partial<OverlayCreate>): void {
    ...
    if (isArray(overlay.points) && overlay.points.length > 0) {
      ...
      if (
        this._overlay.currentStep === OVERLAY_DRAW_STEP_FINISHED &&
        isFunction(this._overlay.performEventPressedMove)
      ) {
  ...
  shouldUpdate(): { draw: boolean; sort: boolean } {
    ...
    const draw =
      sort ||
      JSON.stringify(this._prevOverlay) !==
        JSON.stringify(this._overlay.points) ||
  ...
  eventPressedOtherMove(
    ...
    if (this._prevPressedPoint !== null) {
      ...
      if (
        ...
        isNumber(this._prevPressedPoint.dataIndex)
      ) {
      ...
      this._overlay.points = this._prevPressedPoints.map((p) => {
        ...
        if (isNumber(difDataIndex) && isNumber(p.dataIndex)) {
          ...
          newPoint.timestamp =
            timeScaleStore.dataIndexToTimestamp(newPoint.dataIndex) ??
            undefined;
        }
