...
export type IndicatorFigureAttrsCallbackData<D> =
  IndicatorFigureCallbackBrother<D>;
...
export function eachFigures<D>(
  ...
  figures.forEach((figure) => {
    ...
    if (isValid(defaultFigureStyles)) {
      const cbData = {
        prev: {
          TViewData: TViewDataList[dataIndex - 1],
          indicatorData: result[dataIndex - 1],
        },
        current: {
          TViewData: TViewDataList[dataIndex],
...
export default class IndicatorImp<D = any> {
  ...
  private readonly _indicator: Indicator<D> = {
    ...
    shouldUpdate: (prev, current) => {
      const calc =
        JSON.stringify(prev.calcParams) !==
          JSON.stringify(current.calcParams) ||
  ...
  async calc(dataList: TViewData[]): Promise<boolean> {
    try {
