...
export default abstract class AxisImp
  implements Pick<AxisTemplate, 'createTicks'>, Axis
{
  private readonly _parent: DrawPane<AxisImp>;

  private _range: AxisRange = {
    from: 0,
    to: 0,
    range: 0,
    realFrom: 0,
  ...
  private _extremum: AxisExtremum = {
    min: 0,
    max: 0,
    range: 0,
    realMin: 0,
  ...
  setExtremum(extremum: AxisExtremum): void {
    const canYScroll = this.getParent()
      .getChart()
      .getChartStore()
      .getYScrolling();
  ...
  private _calcTickInterval(range: number): number[] {
    if (this.getParent().getMainWidget().getName() === 'xAxis') {
      ...
      const interval = nice(range / 18.0);
      const precision = getPrecision(interval);
      return [interval, precision];
    }
  }
