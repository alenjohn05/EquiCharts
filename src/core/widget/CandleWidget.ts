...
import CandleHeikinAshiView from '../view/CandleHeikinAshiView';

export default class CandleWidget extends IndicatorWidget {
  private readonly _candleBarView = new CandleBarView(this);
  private readonly _candleHeikinAshiView = new CandleHeikinAshiView(this);
  ...
  override updateMainContent(ctx: CanvasRenderingContext2D): void {
    ...
    if (
      candleStyles.type !== CandleType.Area &&
      candleStyles.type !== CandleType.Line &&
      candleStyles.type !== CandleType.LineMark &&
      candleStyles.type !== CandleType.StepLine
    ) {
      if (candleStyles.type === CandleType.HeikinAshi) {
        this._candleHeikinAshiView.draw(ctx);
      } else {
