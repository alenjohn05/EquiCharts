...
export function checkCoordinateOnArc(
  ...
  for (let i = 0; i < arcs.length; i++) {
    ...
    if (Math.abs(getDistance(coordinate, arc) - arc.r) < DEVIATION) {
      ...
      if (
        coordinate.x <=
          Math.max(startCoordinateX, endCoordinateX) + DEVIATION &&
        coordinate.x >=
          Math.min(startCoordinateX, endCoordinateX) - DEVIATION &&
        coordinate.y <=
          Math.max(startCoordinateY, endCoordinateY) + DEVIATION &&
