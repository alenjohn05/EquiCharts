...
const currentRatio: IndicatorTemplate<Cr> = {
  ...
  calc: (dataList: TViewData[], indicator: Indicator<Cr>) => {
    ...
    dataList.forEach((TViewData: TViewData, i: number) => {
      const cr: Cr = {};
      const prevData = dataList[i - 1] ?? TViewData;
      const prevMid =
        (prevData.high + prevData.close + prevData.low + prevData.open) / 4;

      const highSubPreMid = Math.max(0, TViewData.high - prevMid);

      const preMidSubLow = Math.max(0, prevMid - TViewData.low);
