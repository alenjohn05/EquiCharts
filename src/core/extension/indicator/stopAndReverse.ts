...
const stopAndReverse: IndicatorTemplate<Sar> = {
  ...
  figures: [
    {
      ...
      styles: (
        ...
        const TViewData = current.TViewData!;
        const halfHL = (TViewData?.high + TViewData?.low) / 2;
        const color =
          sar < halfHL
            ? (formatValue(
                indicator.styles,
  ...
  calc: (dataList: TViewData[], indicator: Indicator<Sar>) => {
