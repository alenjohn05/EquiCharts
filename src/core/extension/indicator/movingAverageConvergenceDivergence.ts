  ...
  figures: [
    ...
    { key: 'dea', title: 'DEA: ', type: 'line' },
    {
      ...
      styles: (
        ...
        const currentMacd =
          current.indicatorData?.macd ?? Number.MIN_SAFE_INTEGER;
        let color: string;
        if (currentMacd > 0) {
          color = formatValue(
            indicator.styles,
        ...
        const style =
          prevMacd < currentMacd ? PolygonType.Stroke : PolygonType.Fill;
        return { style, color, borderColor: color };
      },
    },
  ],
  calc: (dataList: TViewData[], indicator: Indicator<Macd>) => {
