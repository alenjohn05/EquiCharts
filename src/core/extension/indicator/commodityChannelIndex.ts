...
const commodityChannelIndex: IndicatorTemplate<Cci> = {
  ...
  calc: (dataList: TViewData[], indicator: Indicator<Cci>) => {
    ...
    return dataList.map((TViewData: TViewData, i: number) => {
      const cci: Cci = {};
      const tp = (TViewData.high + TViewData.low + TViewData.close) / 3;
      tpSum += tp;
      tpList.push(tp);
      if (i >= p) {
        ...
        const agoTp =
          (dataList[i - p].high + dataList[i - p].low + dataList[i - p].close) /
          3;
        tpSum -= agoTp;
      }
      return cci;
