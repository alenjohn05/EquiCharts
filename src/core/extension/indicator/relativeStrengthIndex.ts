...
const relativeStrengthIndex: IndicatorTemplate<Rsi> = {
  ...
  calc: (dataList: TViewData[], indicator: Indicator<Rsi>) => {
    const { calcParams: params, figures } = indicator;
    const sumCloseAs: number[] = [];
    const sumCloseBs: number[] = [];
    return dataList.map((TViewData, i) => {
      const rsi = {};
      const prevClose = (dataList[i - 1] ?? TViewData).close;
      const tmp = TViewData.close - prevClose;
        ...
        if (i >= p - 1) {
          if (sumCloseBs[index] !== 0) {
            rsi[figures[index].key] =
              100 - 100.0 / (1 + sumCloseAs[index] / sumCloseBs[index]);
          } else {
