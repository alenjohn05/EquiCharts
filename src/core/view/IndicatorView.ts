    ...
    indicators.forEach((proxy) => {
      ...
      if (indicator.visible) {
        ...
        if (!isCover) {
          ...
          const lines: Array<
            Array<{ coordinates: Coordinate[]; styles: SmoothLineStyle }>
          > = [];
                ...
                if (isValid(currentData?.[figure.key])) {
                  ...
                  if (!isValid<IndicatorFigureAttrs>(attrs)) {
                    switch (figure.type) {
                      ...
                      case 'bar': {
                        const baseValue =
                          figure.baseValue ?? yAxis.getRange().from;
                      ...
                      case 'line': {
                        ...
                        if (
                          isNumber(currentCoordinate[figure.key]) &&
                          isNumber(nextCoordinate[figure.key])
                        ) {
          ...
          lines.forEach((items) => {
            if (items.length > 1) {
              ...
              for (let i = 1; i < items.length; i++) {
                ...
                const lastMergeLineLastCoordinate =
                  lastMergeLine.coordinates[
                    lastMergeLine.coordinates.length - 1
                  ];
                if (
                  ...
                  lastMergeLine.styles.dashedValue[0] ===
                    current.styles.dashedValue[0] &&
                  lastMergeLine.styles.dashedValue[1] ===
                    current.styles.dashedValue[1]
                ) {
