...
export default class CandleLastPriceView extends View {
  override drawImp(ctx: CanvasRenderingContext2D): void {
    ...
    const lastPriceMartViewStyles = lastPriceMarkStyles.line;
    if (
      priceMarkStyles.show &&
      lastPriceMarkStyles.show &&
      lastPriceMartViewStyles.show
    ) {
      ...
      if (data != null) {
        ...
        this.createFigure({
          ...
          styles: {
            style: lastPriceMartViewStyles.style,
            color,
            size: lastPriceMartViewStyles.size,
            dashedValue: lastPriceMartViewStyles.dashedValue,
          },
        })?.draw(ctx);
      }
    }
