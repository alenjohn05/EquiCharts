...
export default class CandleTooltipView extends IndicatorTooltipView {
  override drawImp(ctx: CanvasRenderingContext2D): void {
    ...
    if (isValid(crosshair.TViewData)) {
      const bounding = widget.getBounding();
      const yAxisBounding = pane.getYAxisWidget()!.getBounding();
      // const headingText = 'Candle and Indicator Information'
      const headingY = this._addHeading(bounding.top + 10);
      const adjustedTop = headingY.height;
      ...
      if (
        ...
        const { offsetLeft, offsetRight } = candleStyles.tooltip;
  ...
  private _addHeading(y: number): { height: number; left: number } {
    // const headingStyles = {
    //   color: '#FFFFFF',
    //   size: 20,
    //   weight: 'bold',
    ...
    //   marginRight: 20
    // }
    // ctx.font = createFont(headingStyles.size, headingStyles.weight, headingStyles.family)
    // ctx.fillStyle = headingStyles.color
    // ctx.textBaseline = 'top'
    // ctx.fillText(text, x, y)
    return { height: y + 30, left: 330 };
  }
    ...
    if (isDrawCandleTooltip || isDrawIndicatorTooltip) {
      ...
      if (rectWidth !== 0 && rectHeight !== 0) {
        ...
        if (isPointer) {
          ...
          if (
            rectY + rectHeight >
            bounding.height - rectOffsetBottom - offsetBottom
          ) {
            rectY =
              bounding.height - rectOffsetBottom - rectHeight - offsetBottom;
          }
    ...
    const mapping = {
      ...
      '{change}':
        prevClose === 0
          ...
          : `${formatThousands(formatPrecision((changeValue / prevClose) * 100), thousandsSeparator)}%`,
    };
    const legends =
