...
export default class ChartStore {
  ...
  adjustVisibleDataList(): void {
    ...
    for (let i = realFrom; i < realTo; i++) {
      const TViewData = this._dataList[i];
  ...
  getVisibleFirstData(): Nullable<TViewData> {
    const { from } = this._timeScaleStore.getVisibleRange();
    return this._dataList[from] ?? null;
  }

  getVisibleLastData(): Nullable<TViewData> {
    const { to } = this._timeScaleStore.getVisibleRange();
    return this._dataList[to] ?? null;
  }
