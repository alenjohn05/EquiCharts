...
export default class TooltipStore {
  ...
  setCrosshair(crosshair?: Crosshair, notInvalidate?: boolean): void {
    ...
    if (isNumber(cr.x)) {
      realDataIndex = this._chartStore
        .getTimeScaleStore()
    ...
    const TViewData: Nullable<TViewData> = dataList[dataIndex];
    const realX = this._chartStore
      .getTimeScaleStore()
    ...
    this._crosshair = { ...cr, realX, TViewData, realDataIndex, dataIndex };
    if (
