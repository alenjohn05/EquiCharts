...
export default class TimeScaleStore {
  ...
  adjustVisibleRange(): void {
    ...
    if (this._scrollLimitRole === ScrollLimitRole.Distance) {
      leftMinVisibleBarCount =
        (this._totalBarSpace - this._maxOffsetDistance.right) / this._barSpace;
      rightMinVisibleBarCount =
        (this._totalBarSpace - this._maxOffsetDistance.left) / this._barSpace;
    } else {
    ...
    const maxRightOffsetBarCount =
      visibleBarCount - Math.min(leftMinVisibleBarCount, totalBarCount);
    ...
    const minRightOffsetBarCount =
      -totalBarCount + Math.min(rightMinVisibleBarCount, totalBarCount);
    ...
    this._chartStore
      .getActionStore()
      .execute(ActionType.OnVisibleRangeChange, this._visibleRange);
  ...
  setTimezone(timezone: string): void {
    const dateTimeFormat: Nullable<Intl.DateTimeFormat> =
      this._buildDateTimeFormat(timezone);
  ...
  setBarSpace(barSpace: number, adjustBeforeFunc?: () => void): void {
    if (
      barSpace < BarSpaceLimitConstants.MIN ||
      barSpace > BarSpaceLimitConstants.MAX ||
  ...
  setOffsetRightDistance(distance: number, isUpdate?: boolean): this {
    this._offsetRightDistance =
      this._scrollLimitRole === ScrollLimitRole.Distance
        ? Math.min(this._maxOffsetDistance.right, distance)
        : distance;
    this._lastBarRightSideDiffBarCount =
      this._offsetRightDistance / this._barSpace;
    if (isUpdate ?? false) {
  ...
  startScroll(): void {
    this._startLastBarRightSideDiffBarCount =
      this._lastBarRightSideDiffBarCount;
  }
  ...
  scroll(distance: number): void {
    ...
    const prevLastBarRightSideDistance =
    ...
    if (realDistance !== 0) {
      this._chartStore
        .getActionStore()
        .execute(ActionType.OnScroll, { distance: realDistance });
    }
  }

  getDataByDataIndex(dataIndex: number): Nullable<TViewData> {
  ...
  coordinateToFloatIndex(x: number): number {
    ...
    const index =
      dataCount + this._lastBarRightSideDiffBarCount - deltaFromRight;
  ...
  zoom(scale: number, coordinate?: Partial<Coordinate>): void {
    ...
    const barSpace =
      this._barSpace + scale * (this._barSpace / SCALE_MULTIPLIER);
    this.setBarSpace(barSpace, () => {
      this._lastBarRightSideDiffBarCount +=
        floatIndex - this.coordinateToFloatIndex(x);
    });
    const realScale = this._barSpace / prevBarSpace;
    if (realScale !== 1) {
      this._chartStore
        .getActionStore()
        .execute(ActionType.OnZoom, { scale: realScale });
    }
  }
