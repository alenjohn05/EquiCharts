...
import {
  ...
  TooltipIconPosition, ActionType, PaneOptions, Indicator, DomPosition, DeepPartial,
  registerOverlay
} from  'equicharts'
...
import DatepickerModel from './widget/datepicker-model'
import { addPeriodsToTimestamp } from './extension/utils'
import { createButtonsContainer, createLogarithmicButton, createPercentageButton, createPriceUnitElement, createSymbolHeading, createWatermark } from './base/mountHtmlFunctions'
...
const ChartProComponent: Component<ChartProComponentProps> = props => {
  ...
  let logarthimic: HTMLElement
  let percentage: HTMLElement
  let symbolHeadingElement: HTMLElement
  ...
  const [goToCalenderModelVisible, setGoToCalenderModelVisible] = createSignal(false)
  ...
  const [selectedCandlestickType, setselectedCandlestickType] = createSignal<string>("")
  const [selectedPriceDataSource, setselectedPriceDataSource] = createSignal<string>("normal")
  const [isLogarithmic, setIsLogarithmic] = createSignal(false)
  ...
  onMount(() => {
    ...
    widget = init(widgetRef!, getInitialOptions(period()))
    if (widget) {
      const watermarkContainer = widget.getDom('candle_pane', DomPosition.Main)
      const symbolHeadingElementContainer = widget.getDom('candle_pane', DomPosition.Main)
      if (symbolHeadingElementContainer) {
        symbolHeadingElement = document.createElement('div');
        symbolHeadingElement.className = 'tv-symbol-heading';
        symbolHeadingElement.innerHTML = createSymbolHeading(symbol());
        symbolHeadingElementContainer.appendChild(symbolHeadingElement);
      }
      if (watermarkContainer) {
        const watermark = createWatermark(props);
      ...
      logarthimic = createLogarithmicButton(setIsLogarithmic, isLogarithmic, isPercentage, handleSelectedYAxisDataSource);
      percentage = createPercentageButton(setIsPercentage, isPercentage, isLogarithmic, logarthimic, handleSelectedYAxisDataSource);
    ...
    widget?.loadMore((timestamp: Nullable<number>) => {
      loading = true
      const get = async () => {
        const p = period()
  ...
  onCleanup(() => {
    ...
    if (logarthimic) {
      logarthimic.onclick = null
    }
    if (percentage) {
      percentage.onclick = null
    }
    dispose(widgetRef!)
  })
  ...
  createEffect(() => {
    if (styles()) {
      ...
      const style = lodashClone(widget!.getStyles())
      setselectedCandlestickType(style.candle.type)
      setWidgetDefaultStyles(style)
    }
  })

  createEffect(() => {
    const currentSymbol = symbol();
    if (symbolHeadingElement) {
      symbolHeadingElement.innerHTML = createSymbolHeading(currentSymbol);
    }
  });
  ...
  const handleMoveToATimeStamp = (timeData: number) => {
    ...
    const overlayid = widget?.createOverlay('priceLine2', 'candle_pane')
    function sayHello() {
      if (typeof overlayid === 'string') {
        console.log(overlayid)
        widget?.removeOverlay(overlayid)
      }
    }
    setTimeout(sayHello, 2000)
  }

  const handleSelectedYAxisDataSource = (style: string | DeepPartial<Styles>) => {
  ...
  return (
    <>
      ...
      <Show when={goToCalenderModelVisible()}>
        <DatepickerModel
          locale={props.locale}
          onClose={() => { setGoToCalenderModelVisible(false) }}
          onConfirm={handleMoveToATimeStamp} />
      </Show>
      ...
      <PeriodBar
        ...
        currentCandleType={selectedCandlestickType()}
        onMenuClick={async () => {
          try {
        ...
        onScreenshotClick={() => {
          if (widget) {
            const url = widget.getConvertPictureUrl(true, 'jpeg', props.theme === 'dark' ? '#151517' : '#e5e5e5')
            setScreenshotUrl(url)
          }
        }}
        onCandleTypeChange={(style, type) => {
          widget?.setStyles(style)
      ...
      <div class='klinecharts-pro-footer'>
        <Footer
          onTimezoneClick={() => { setTimezoneModalVisible((visible => !visible)) }}
          onMoveToClick={() => { setGoToCalenderModelVisible((visible => !visible)) }}
          timezone={timezone()}
          locale={props.locale}
        />
      </div>
