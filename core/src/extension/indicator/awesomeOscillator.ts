  ...
  figures: [
    {
      ...
      styles: (
        ...
        const style =
          currentAo > prevAo ? PolygonType.Stroke : PolygonType.Fill;
        return { color, style, borderColor: color };
      },
    },
  ],
  calc: (dataList: TViewData[], indicator: Indicator<Ao>) => {
    ...
    return dataList.map((TViewData: TViewData, i: number) => {
      const ao: Ao = {};
      const middle = (TViewData.low + TViewData.high) / 2;
      shortSum += middle;
      longSum += middle;
      if (i >= params[0] - 1) {
        short = shortSum / params[0];
        const agoTViewData = dataList[i - (params[0] - 1)];
        shortSum -= (agoTViewData.low + agoTViewData.high) / 2;
      }
      if (i >= params[1] - 1) {
        long = longSum / params[1];
        const agoTViewData = dataList[i - (params[1] - 1)];
        longSum -= (agoTViewData.low + agoTViewData.high) / 2;
      }
      if (i >= maxPeriod - 1) {
        ao.ao = short - long;
      }
