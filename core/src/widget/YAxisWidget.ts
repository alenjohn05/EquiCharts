...
import CandleVisibleLastPriceLabelView from '../view/CandleVisibleLastPriceLabelView';
...
export default class YAxisWidget extends DrawWidget<DrawPane<YAxis>> {
  ...
  private readonly _crosshairHorizontalLabelView =
    new CrosshairHorizontalLabelView(this);
  ...
  override updateMain(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.getPane().getAxisComponent().isInCandle()) {
      ...
      this._candleVisibleLastPriceLabelView.draw(ctx);
    }
