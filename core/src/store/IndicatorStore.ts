...
export default class IndicatorStore {
  ...
  private _sort(paneId?: string): void {
    if (isString(paneId)) {
      this._instances
        .get(paneId)
        ?.sort((i1, i2) => i1.getIndicator().zLevel - i2.getIndicator().zLevel);
    } else {
  ...
  getInstanceByPaneId(
    ...
    if (isString(paneId)) {
      ...
      if (isString(name)) {
        return (
          paneInstances
            ?.find((ins) => ins.getIndicator().name === name)
            ?.getIndicator() ?? null
        );
      }
  ...
  synchronizeSeriesPrecision(indicator?: IndicatorImp): void {
    const { price: pricePrecision, volume: volumePrecision } =
      this._chartStore.getPrecision();
