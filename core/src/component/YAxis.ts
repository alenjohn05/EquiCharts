...
export default abstract class YAxisImp extends AxisImp implements YAxis {
  protected calcRange(): AxisRange {
    ...
    const isLine = candleStyles.type === CandleType.Line;
    const isLineMark = candleStyles.type === CandleType.LineMark;
    const valueKey = isArea ? candleStyles.area.value : candleStyles.line.value;
  ...
  override getAutoSize(): number {
    ...
    if (yAxisStyles.show) {
      ...
      if (yAxisStyles.tictView.show) {
        yAxisWidth += yAxisStyles.tictView.length;
      }
