...
export type TooltipTextStyle = Pick<
  TextStyle,
...
export interface CandleLinePointStyle {
  show: boolean;
  color: string;
  radius: number;
  rippleColor: string;
...
export interface CandleHighLowStyle {
  color: string;
}
...
export type CandleLastPriceMartViewStyle = Omit<StateLineStyle, 'color'>;
...
export interface CandleVisiblePriceMarkStyle {
  show: boolean;
  high: CandleHighLowPriceMarkStyle;
  low: CandleHighLowPriceMarkStyle;
  last: CandleLastPriceMarkStyle;
...
export type IndicatorPolygonStyle = Omit<
  PolygonStyle,
  'color' | 'borderColor'
> &
  ChangeColor;
...
export interface AxisTictViewStyle extends AxisLineStyle {
  length: number;
}
...
function getDefaultCandleStyle(): CandleStyle {
  ...
  return {
    ...
    visiblePriceMark: {
      show: true,
      high: { ...highLow, color: green },
      low: { ...highLow, color: red },
      last: {
        show: true,
        upColor: green,
        downColor: red,
...
function getDefaultIndicatorStyle(): IndicatorStyle {
  const lines = ['#FF9600', '#935EBD', blue, '#E11D74', '#01C5C4'].map(
    (color) => ({
