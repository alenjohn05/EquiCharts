...
export default class CandleBarView extends ChildrenView {
  ...
  override drawImp(ctx: CanvasRenderingContext2D): void {
    ...
    if (candleBarOptions !== null) {
      let ohlcSize = 0;
      let halfOhlcSize = 0;
      let maxMin: number[] = [1, 1];
      if (candleBarOptions.type === CandleType.CandleVolume) {
        const data = chartStore.getVisibleDataList();
        const calcInstance = data
          .filter((each) => each.data !== undefined && each.data !== null)
          .map((each) => each.data!);
        maxMin = getMaxMin(calcInstance, 'volume', 'volume');
      }
      ...
      this.eachChildren((data, barSpace) => {
        ...
        if (isValid(TViewData)) {
          const { open, high, low, close, volume } = TViewData;
          ...
          switch (type) {
            ...
            case CandleType.CandleHighLow: {
              const color = chartStore.getStyles().candle.highLow.color;
              rects = this._createHighLowCandle(x, priceY, barSpace, color);
              break;
            }
  ...
  protected getCandleBarOptions(
    ...
    return {
      type: candleStyles.type as Exclude<
        CandleType,
        | CandleType.Area
        | CandleType.Line
        | CandleType.LineMark
  ...
  private _createVolumeCandleBar(
    x: number,
    priceY: number[],
    barSpace: BarSpace,
    colors: string[],
    ...
    return [
      {
        ...
        styles: { color: colors, borderColor: colors },
      },
    ];
  }

  private _createStrokeBar(
    x: number,
    priceY: number[],
    barSpace: BarSpace,
    colors: string[],
