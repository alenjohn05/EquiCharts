...
export default class IndicatorTooltipView extends View<YAxis> {
  private readonly _boundIconClickEvent = (currentIcon: TooltipIcon) => () => {
    ...
    pane
      ...
      .getChartStore()
      .getActionStore()
      .execute(ActionType.OnTooltipIconClick, { ...currentIcon });
    return true;
  };

  private readonly _boundIconMouseMoveEvent =
    (currentIconInfo: TooltipIcon) => () => {
      const pane = this.getWidget().getPane();
  ...
  protected drawStandardTooltipIcons(
    ...
    if (icons.length > 0) {
      ...
      icons.forEach((icon) => {
        ...
        const active =
          activeIcon?.paneId === paneId &&
          activeIcon?.indicatorName === indicatorName &&
