...
export default class Event implements EventHandler {
  ...
  private readonly _boundKeyBoardDownEvent: (event: KeyboardEvent) => void = (
    ...
    if (event.shiftKey) {
      switch (event.code) {
        ...
        case 'ArrowLeft': {
          const timeScaleStore = this._chart
            .getChartStore()
            .getTimeScaleStore();
  ...
  pinchEvent(e: MouseTouchEvent, scale: number): boolean {
    ...
    if (
      ...
      this._chart
        .getChartStore()
        .getTimeScaleStore()
        .zoom(zoomScale, { x: event.x, y: event.y });
      return true;
    }
    return false;
  }
  ...
  mouseWheelVertEvent(e: MouseTouchEvent, scale: number): boolean {
    ...
    if (name === WidgetNameConstants.MAIN) {
      this._chart
        .getChartStore()
        .getTimeScaleStore()
        .zoom(scale, { x: event.x, y: event.y });
      return true;
    }
    return false;
  }
    ...
    if (widget !== null) {
      ...
      switch (name) {
        ...
        case WidgetNameConstants.MAIN: {
          const range =
            (pane as DrawPane<YAxis>).getAxisComponent().getRange() ?? null;
    ...
    if (widget !== null) {
      ...
      switch (name) {
        case WidgetNameConstants.MAIN: {
          ...
          if (
            consumed &&
            chartStore.getTooltipStore().getActiveIcon() !== null
          ) {
            crosshair = undefined;
            if (widget !== null) {
        ...
        case WidgetNameConstants.MAIN: {
          ...
          if (!consumed && this._startScrollCoordinate !== null) {
            ...
            if (
              ...
              !yAxis.getAutoCalcTickFlag() &&
              yAxis.getScrollZoomEnabled()
            ) {
          ...
          this._chart
            .getChartStore()
            .getTooltipStore()
            .setCrosshair({ x: event.x, y: event.y, paneId: pane?.getId() });
          return consumed;
        }
        case WidgetNameConstants.X_AXIS: {
          const consumed = widget.dispatchEvent('pressedMouseMoveEvent', event);
      ...
      switch (name) {
        case WidgetNameConstants.MAIN: {
          ...
          if (this._touchCoordinate !== null) {
            ...
            if (
              this._startScrollCoordinate !== null &&
              Math.abs(this._startScrollCoordinate.x - event.x) >
                this._startScrollCoordinate.y - event.y
            ) {
              const distance = event.x - this._startScrollCoordinate.x;
  ...
  private _findWidgetByEvent(event: MouseTouchEvent): EventTriggerWidgetInfo {
    ...
    const separatorSize = this._chart.getChartStore().getStyles()
      .separator.size;
    for (const [, pane] of separatorPanes) {
      const bounding = pane.getBounding();
      const top =
        bounding.top - Math.round((REAL_SEPARATOR_HEIGHT - separatorSize) / 2);
      if (
        x >= bounding.left &&
        x <= bounding.left + bounding.width &&
        y >= top &&
        y <= top + REAL_SEPARATOR_HEIGHT
      ) {
